...
ref_t<MClientRequest> Client::build_client_request(MetaRequest *request)
{
  ...
  auto session = mds_sessions.at(request->mds);
  if ((session->mds_features.test(CEPHFS_FEATURE_32BITS_RETRY_FWD) &&
       request->retry_attempt >= old_max_retry) ||
      request->retry_attempt >= UINT32_MAX - 1) {
...
void Client::handle_client_request_forward(const MConstRef<MClientRequestForward>& fwd)
{
  ...
  if (num_fwd <= request->num_fwd || num_fwd >= UINT32_MAX - 1) {
    request->abort(-EMULTIHOP);
    request->caller_cond->notify_all();
